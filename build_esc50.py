...
def parse_single_audio_file(y, sr, label, filename):
    data = {
        "sr": _int64_feature(sr),  # sampling rate
        "len": _int64_feature(len(y)),  # required to reshape when reading
        "y": _bytes_feature(serialize_array(y)),  # the audio data
        "label": _int64_feature(label),  # the label
        "filename": _bytes_feature(serialize_array(filename)),  # the original file name
    }
...
def main(args):
    ...
    for fold_number in range(1, 6):
        current_pattern = os.path.join(args.dataset_dir, f"{fold_number}-*.wav")
        print(f"Currently parsing fold {fold_number} with pattern {current_pattern}")
        # find all audio files that match the current search pattern
